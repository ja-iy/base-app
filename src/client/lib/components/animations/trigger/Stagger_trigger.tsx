...
import { useSpring, useTrail, useInView, animated, config as preset_configs, easings, useTransition } from '@react-spring/web'
...
interface Stagger_TriggerProps {
...
export default function Stagger_Trigger({trigger, once, className, style, no_fade, no_fade_in, no_fade_out, from, from_len, to, to_len, len, config, t, easing, changeOn, bind, children}:Stagger_TriggerProps){
    ...
    return <>
        <div className={className} style={{...style}} {...bind()} >
            {trails.map((springs, i) => <animated.div style={{...springs}} key={i}>
                {children[i]}
            </animated.div>)}
        </div>
    </>
